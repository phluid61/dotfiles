...
" MVK
syn keyword mysqlSpecial InnoDB MyISAM
syn keyword mysqlSpecial REDUNDANT COMPACT DYNAMIC COMPRESSED
...
syn keyword mysqlKeyword         cascade change character charset check checksum collate column columns comment constraint create cross
...
syn keyword mysqlKeyword         enclosed engine escape escaped explain
...
syn keyword mysqlKeyword         read references reload rename replace restrict returns revoke right row rows row_format
syn keyword mysqlKeyword         second select set show shutdown soname sql_big_result sql_big_selects sql_big_tables sql_log_off
...
syn keyword mysqlOperator        between not and or if is in like regexp rlike binary exists
